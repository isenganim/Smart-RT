...
rules(['status' => ['required', 'in:diajukan,disetujui,ditolak,selesai'], 'notes' => ['nullable', 'string', 'max:2000']]);
...
$saveUpdate = function () { $this->validate(); $item = LetterRequest::findOrFail($this->updateId); $from = $item->status->value; $item->update(['status' => $this->status, 'notes' => $this->notes]); Audit::record(auth()->user(), 'letter.status_changed', 'letter_request', $item->id, ['from' => $from, 'to' => $this->status]); $this->reset('updateId', 'status', 'notes'); };
?>
