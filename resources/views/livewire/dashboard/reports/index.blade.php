...
rules(['status' => ['required', 'in:baru,diproses,selesai,ditolak'], 'notes' => ['nullable', 'string', 'max:2000']]);
...
$saveUpdate = function () { $this->validate(); $item = Report::findOrFail($this->updateId); $from = $item->status->value; $item->update(['status' => $this->status, 'notes' => $this->notes]); Audit::record(auth()->user(), 'report.status_changed', 'report', $item->id, ['from' => $from, 'to' => $this->status]); $this->reset('updateId', 'status', 'notes'); };
?>
